...
int FcComd(int);
int ReplaceComd(int);
int MoveComd(int);     // move命令处理函数
int BatchComd(int);    // batch命令处理函数
...
void ExecComd(int k)		//执行命令
{
 ...
 "undel","Prompt","udtab","fc","replace","move","batch" };
	...
	case 25:UdTabComd();		//udtab命令，显示被删除文件表(调试程序用)
		break;
	case 26:FcComd(k);          // ← 新增这一行！fc 命令
		break;
	case 27:ReplaceComd(k);                      // ← 新增这一行！fc 命令
		break;
	case 28: MoveComd(k);
		break;
	case 29: BatchComd(k);
...
void HelpComd()				//help命令，帮助信息(显示各命令格式)
{
	...
	cout << "move <文件/目录> <目录名>              ——移动文件/目录；或对子目录改名。\n";
	cout << "batch <batchfile>                     ——从外部文本文件逐行执行命令。\n";
...
int MdComd(int k)		// md命令处理函数
{
	...
	// ===================== 核心修复点 =====================
	// 修改前：i = FindFCB(DirName, s, attrib, p); // 仅查找同名目录
	// 修改后：将属性参数设为 '\0'，查找所有属性的同名项（包括文件和目录）
	i = FindFCB(DirName, s, '\0', p);

	if (i >= 0) // 如果返回值 >= 0，说明找到了同名项
	{
		cout << "\n错误：目录与现有目录或文件重名！\n";
		return -1;
	}
	...
	s0 = M_NewDir(DirName, p, s, attrib); // 创建新目录项
	if (s0 < 0)
	{
		cout << "\n磁盘空间已满，创建目录失败。\n";
		return -1;
...
int TypeComd(int k)
{
	...
	if (k == 0)
	{
		if (cur_uof == -1 || uof[cur_uof].state == 0)
		{
			cout << "\n当前没有操作文件。\n";
			return -1;
		}
	...
	s = ProcessPath(comd[1], FileName, k, 0, attrib);
	if (s < 1)
		return s;
...
short int SAVE_bn(short bb)
{
	...
	short head = getblock();        // 链表的第一个节点
	short curr = head;              // 当前正在写的块
	short* p = (short*)Disk[curr];
	int idx = 0;
...
int DelComd(int k)
{
	...
	if (strcmp(comd[1], "*") == 0)
	{
		...
		{
			dir_fcbp = (FCB*)Disk[s];
			for (i = 0; i < 4; i++, dir_fcbp++)
			{
				if (dir_fcbp->FileName[0] == '\0')  // 目录项结束
					break;
				if (dir_fcbp->FileName[0] == (char)0xe5)  // 已删除
					continue;

				// 跳过目录和隐藏文件
				if (dir_fcbp->Fattrib >= 16)  // 是目录
					continue;

				// 先保存文件名，因为后面会被修改
				char filename[FILENAME_LEN];
				strcpy(filename, dir_fcbp->FileName);
				...
				int uofIndex = Check_UOF(fullpath);
				if (uofIndex != S && uof[uofIndex].state == 2)
				{
					cout << "跳过文件: " << filename << " (文件正在使用)" << endl;
					continue;
				}

				// 释放文件占用的盘块
				short block = dir_fcbp->Addr;
				while (block > 0)
				{
					short next = FAT[block];
					FAT[block] = 0;  // 标记为空闲
					FAT[0]++;        // 空闲块数增加
					block = next;
				}
	...
	else if (strchr(comd[1], '*') != NULL)
	{
		// 处理其他通配符模式（如 *.txt）
		// 这里简单实现，实际需要更复杂的模式匹配
		cout << "\n暂不支持复杂的通配符模式，仅支持 '*'。" << endl;
		return -1;
	}

	// 原有删除单个文件的代码
	s0 = ProcessPath(comd[1], FileName, k, 0, attrib);
	if (s0 < 1) return s0;

	fcbp = NULL;
	s = FindFCB(FileName, s0, attrib, fcbp);
	if (s < 0)
	{
		cout << "\n文件不存在。\n";
		return -1;
	...
	int i_path = strlen(temppath);
	if (temppath[i_path - 1] != '/')
		strcat(fullpath, "/");
	strcat(fullpath, FileName);

	int uofIndex = Check_UOF(fullpath);
	if (uofIndex != S && uof[uofIndex].state != 0)
	{
		cout << "\n文件正在打开，不能删除。\n";
		return -1;
	}
...
int CopyComd(int k)   // 统一 copy 命令处理函数
{
	short int i, size, s01, s02, s1, s2, s22;
	short int b, b0, bnum;
	char attrib = '\0', * FileName1, * FileName2;
	char gFileName[PATH_LEN], yn;
	char saved_temppath[PATH_LEN];
	...
	if (strcmp(comd[1], "*") == 0)
	{
		...
		strcpy(saved_temppath, temppath);

		s02 = ProcessPath(comd[2], FileName2, k, 0, '\20');
		if (s02 < 1)
		{
			strcpy(temppath, saved_temppath);
			return -1;
		}

		FCB* dstFCB;
		if (FindFCB(FileName2, s02, '\040', dstFCB) < 0 || dstFCB->Fattrib != '\20')
		{
			cout << "\n目标不是目录。\n";
			strcpy(temppath, saved_temppath);
			return -1;
		...
		while (curBlk > 0)
		{
			FCB* dirFCB = (FCB*)Disk[curBlk];

			for (int idx = 0; idx < 4; idx++)
			{
				if (dirFCB[idx].FileName[0] == '\0') break;
				if (dirFCB[idx].FileName[0] == (char)0xe5) continue;
				if (dirFCB[idx].Fattrib >= 16) continue;

				char* filename = dirFCB[idx].FileName;

				FCB* tmp;
				if (FindFCB(filename, dstFCB->Addr, '\040', tmp) >= 0)
				{
					skipped++;
					continue;
				}

				if (FindBlankFCB(dstFCB->Addr, fcbp2) < 0)
				{
					failed++;
					continue;
				}

				size = dirFCB[idx].Fsize;
				bnum = size / SIZE + (size % SIZE > 0);
				if (FAT[0] < bnum)
				{
					fcbp2->FileName[0] = (char)0xe5;
					failed++;
					continue;
				}

				*fcbp2 = dirFCB[idx];
				strcpy(fcbp2->FileName, filename);
				fcbp2->Addr = 0;

				short src = dirFCB[idx].Addr;
				short last = 0;

				while (src > 0)
				{
					b = getblock();
					memcpy(Disk[b], Disk[src], SIZE);
					if (last == 0) fcbp2->Addr = b;
					else FAT[last] = b;
					last = b;
					src = FAT[src];
				}
				if (last > 0) FAT[last] = -1;
	...
	strcpy(gFileName, temppath);
	if (temppath[strlen(temppath) - 1] != '/') strcat(gFileName, "/");
...
int ParseCommand(char* p)    //将输入的命令行分解成命令和参数等
{
	...
	for (i = 0; i < len; i++) {
		// 如果是命令后的/或重定向符号，在前面加空格
		if (i > 0 && p[i] == '/' && !isspace(p[i - 1]) &&
			(i == 1 || !isspace(p[i - 2]))) {
			temp[temp_index++] = ' ';
			temp[temp_index++] = p[i];
		}
		// 处理cd..的情况
			...
			temp[temp_index++] = ' ';
			temp[temp_index++] = p[i];
			temp[temp_index++] = p[++i]; // 跳过第二个点
		}
		// 处理重定向符号>和>>
		else if ((p[i] == '>' && !isspace(p[i - 1])) ||
			(p[i] == '>' && p[i + 1] == '>' && !isspace(p[i - 1]))) {
			temp[temp_index++] = ' ';
			temp[temp_index++] = p[i];
			if (p[i + 1] == '>') {
				temp[temp_index++] = p[++i];
	...
	// 处理特殊情况：如果comd[0]以特殊符号开头，可能是重定向
	// 这里主要处理type/usr/boy>>/test这种情况
	for (i = 1; i <= k; i++) {
		// 检查参数中是否包含重定向符号
		char* redirect_pos;
		if ((redirect_pos = strstr(comd[i], ">>")) != NULL) {
			// 分割参数和重定向部分
			char temp_arg[256];
			strcpy(temp_arg, comd[i]);
			*redirect_pos = '\0';
			...
			// 设置重定向部分
			strcpy(comd[i + 1], redirect_pos);
			comd[i + 1][2] = '\0'; // ">>"

			// 设置文件名部分
			strcpy(comd[i + 2], redirect_pos + 2);

			k += 2;
			break;
		}
		else if ((redirect_pos = strchr(comd[i], '>')) != NULL) {
			// 分割参数和重定向部分
			char temp_arg[256];
			strcpy(temp_arg, comd[i]);
			*redirect_pos = '\0';
			...
			// 设置文件名部分
			strcpy(comd[i + 2], redirect_pos + 1);
...
int ReplaceComd(int k)
{
	...
	char attrib = '\0';
	char* SrcFileName, * DstDirName;
	char srcFullPath[PATH_LEN];   // 源文件全路径（用于输出和检查是否同一文件）
	char dstFullPath[PATH_LEN];   // 目标目录全路径
	FCB* srcFCB, * dstFCB, * sameFCB;

	// 1. 解析源文件（必填）
	short srcDirBlock = ProcessPath(comd[1], SrcFileName, k, 0, '\020');
	if (srcDirBlock < 1) return srcDirBlock;

	short srcBlock = FindFCB(SrcFileName, srcDirBlock, attrib, srcFCB);
	if (srcBlock < 0) {
		// ... 不存在报错 ...
	}

	// ↓↓↓↓↓ 关键修复：使用 temppath 而不是 curpath.cpath
	strcpy(srcFullPath, "C:");
	strcat(srcFullPath, temppath);  // ← 改这里！
	if (temppath[strlen(temppath) - 1] != '/') strcat(srcFullPath, "/");
	strcat(srcFullPath, SrcFileName);

	// 2. 解析目标目录（可缺省）
	short dstDirBlock;
	if (k == 1) {                     // 缺省 → 当前目录
		dstDirBlock = curpath.fblock;
		strcpy(dstFullPath, curpath.cpath);
	}
	else {
		dstDirBlock = ProcessPath(comd[2], DstDirName, k, 0, (char)16); // 必须是目录
		if (dstDirBlock < 1) return dstDirBlock;
		strcpy(dstFullPath, temppath);  // temppath 里已经是完整的目标目录路径
	}

	// 3. 在目标目录中查找同名文件
	short sameBlock = FindFCB(SrcFileName, dstDirBlock, attrib, sameFCB);
	if (sameBlock < 0) {
		cout << "\n目录" << (k == 1 ? "当前" : comd[2]) << "中没有可取代的同名文件。\n";
		return -3;
	}

	// 构造被取代文件的完整路径（用于输出和后面判断是否同一文件）
	char targetFullPath[PATH_LEN];
	strcpy(targetFullPath, "C:");
	strcat(targetFullPath, (k == 1 ? curpath.cpath : temppath));
	if (targetFullPath[strlen(targetFullPath) - 1] != '/') strcat(targetFullPath, "/");
	strcat(targetFullPath, SrcFileName);

	// 4. 不能自己取代自己
	if (strcmp(srcFullPath, targetFullPath) == 0) {
		cout << "\n文件不能自己取代自己。\n";
		return -4;
	}

	// 5. 隐藏或系统属性文件不能被取代
	char attr = sameFCB->Fattrib & '\06';   // 隐藏位'\02' + 系统位'\04'
	if (attr != 0) {
		cout << "\n具有隐藏或系统属性的文件不能被取代。\n";
		return -5;
	}
	...
	// 6. 只读文件要询问用户
	if (sameFCB->Fattrib & '\01') {         // 只读属性
		char yn;
		cout << "\n文件" << targetFullPath << "是只读文件，是否仍要取代？(y/n) ";
		cin >> yn;
		if (yn != 'y' && yn != 'Y') {
			cout << "操作已取消。\n";
			return 0;
		}
		cin.ignore();   // 吃掉回车，防止后续命令出错
	}

	// 7. 执行取代：直接把源文件的 FCB 内容复制到目标文件的 FCB
	//    （包括首块号、文件长度、属性等），原目标文件磁盘空间会被回收
	short oldFirstBlock = sameFCB->Addr;      // 记住原来占用的磁盘块链
	*sameFCB = *srcFCB;                       // 整个目录项覆盖（属性也一起复制）
	strcpy(sameFCB->FileName, SrcFileName);   // 确保文件名不变

	// 回收原来目标文件占用的磁盘块（如果不是空文件）
	if (oldFirstBlock > 0) {
		releaseblock(oldFirstBlock);
	}
...
int FcComd(int k) {
	...
	// 处理第一个文件
	short s1 = ProcessPath(comd[1], FileName1, k, 0, '\020');
	if (s1 < 1) return s1;
	s1 = FindFCB(FileName1, s1, attrib, fcbp1);
	if (s1 < 0) {
		strcpy(gFileName1, "C:");
		strcat(gFileName1, temppath);
		if (temppath[strlen(temppath) - 1] != '/') strcat(gFileName1, "/");
		strcat(gFileName1, FileName1);
		cout << "\n文件" << gFileName1 << "不存在。\n";
		return -2;
	}

	// 处理第二个文件
	short s2 = ProcessPath(comd[2], FileName2, k, 0, '\020');
	if (s2 < 1) return s2;
	s2 = FindFCB(FileName2, s2, attrib, fcbp2);
	if (s2 < 0) {
		strcpy(gFileName2, "C:");
		strcat(gFileName2, temppath);
		if (temppath[strlen(temppath) - 1] != '/') strcat(gFileName2, "/");
		strcat(gFileName2, FileName2);
		cout << "\n文件" << gFileName2 << "不存在。\n";
		return -3;
	}
	...
	// 构造完整路径用于输出
	strcpy(gFileName1, "C:");
	strcat(gFileName1, curpath.cpath);
	if (curpath.cpath[strlen(curpath.cpath) - 1] != '/') strcat(gFileName1, "/");
	strcat(gFileName1, FileName1);

	strcpy(gFileName2, "C:");
	strcat(gFileName2, curpath.cpath);
	if (curpath.cpath[strlen(curpath.cpath) - 1] != '/') strcat(gFileName2, "/");
	strcat(gFileName2, FileName2);

	// 读取内容
	char* buf1 = new char[fcbp1->Fsize + 2];
	char* buf2 = new char[fcbp2->Fsize + 2];
	int len1 = file_to_buffer(fcbp1, buf1);
	int len2 = file_to_buffer(fcbp2, buf2);

	// 先比较长度
	if (len1 != len2) {
		int pos = (len1 < len2 ? len1 : len2) + 1;
		char ch1 = (len1 < len2 ? 0 : buf1[pos - 1]);
		char ch2 = (len2 < len1 ? 0 : buf2[pos - 1]);
		cout << "\n文件" << gFileName1 << "与文件" << gFileName2
			<< "在第" << pos << "字节处不同：";
		if (len1 < len2) cout << "EOF"; else cout << ch1;
		cout << "，";
		if (len2 < len1) cout << "EOF"; else cout << ch2;
		cout << endl;
	...
	for (int i = 0; i < len1; i++) {
		if (buf1[i] != buf2[i]) {
			cout << "\n文件" << gFileName1 << "与文件" << gFileName2
				<< "在第" << (i + 1) << "字节处不同：" << buf1[i] << "，" << buf2[i] << endl;
			delete[] buf1; delete[] buf2;
			return 1;
		}
	}
...
int MoveComd(int k)
{
	...
	char attrib_any = '\040'; // 32: 文件/目录都可找
	char attrib_file = '\0';
	char attrib_dir = '\020';

	char* SrcName, * DstName;
	FCB* srcFCB = 0, * dstExistFCB = 0, * dstSameNameFCB = 0;

	// ---------- 1) 定位源对象：得到“源父目录首块号 srcParent” + “源目录项指针 srcFCB” ----------
	short srcParent = ProcessPath(comd[1], SrcName, k, 0, attrib_any);
	if (srcParent < 1) return srcParent;

	short srcAddr = FindFCB(SrcName, srcParent, attrib_any, srcFCB);
	if (srcAddr < 0) {
		cout << "\n要移动的文件或目录不存在。\n";
		return -2;
	}

	bool srcIsDir = (srcFCB->Fattrib >= '\020'); // 目录项属性>=16 认为是目录
	bool srcIsFile = !srcIsDir;

	// 若源是文件且已打开，不能move
	if (srcIsFile) {
		// 构造全路径（FindPath 会把全路径放到 temppath，包含最后名字）
		FindPath(comd[1], attrib_file, 0, dstExistFCB);
		int iu = Check_UOF(temppath);
		if (iu < S) {
			cout << "\n文件" << temppath << "已经打开，不能移动!\n";
			return -3;
		}
	}
	...
	// ---------- 2) 处理目标：分两类 ----------
	// A: 目标存在且是目录 => 移动进该目录
	// B: 目标不存在 => 仅允许 “源是目录 && 同一父目录” 的“目录改名”

	// 先尝试把 comd[2] 当成“目录路径”去找
	FCB* dstDirFCB = 0;
	short dstDirBlock = FindPath(comd[2], attrib_dir, 1, dstDirFCB);

	if (dstDirBlock > 0) {
		// ========= A) 目标目录存在：移动到 dstDirBlock =========

		// 不能“移动到自己里面”这种复杂检测可不做（实验通常不考），但你可以加防护。

		// 在目标目录中查同名项（文件或目录都算）
		short same = FindFCB(SrcName, dstDirBlock, attrib_any, dstSameNameFCB);

		if (same >= 0) {
			// 目标中存在同名项
			bool dstIsDir = (dstSameNameFCB->Fattrib >= '\020');
			...
			// 目标同名是“文件”：允许覆盖（目录也可覆盖文件）
			char yn[INPUT_LEN];
			cout << "\n存在同名文件，是否要覆盖它？(Yes/No) ";
			cin.getline(yn, INPUT_LEN);
			if (!(_stricmp(yn, "yes") == 0 || _stricmp(yn, "y") == 0)) {
				cout << "操作已取消。\n";
				return 0;
			}

			// 回收被覆盖文件的数据块
			if (dstSameNameFCB->Addr > 0) releaseblock(dstSameNameFCB->Addr);

			// 用“转交”方式覆盖：把源目录项整体搬过去（名字保持不变）
			*dstSameNameFCB = *srcFCB;
			strcpy(dstSameNameFCB->FileName, SrcName);
		}
		else {
			// 目标目录中无同名项 => 新建一个目录项
			FCB* blank;
			int ok = FindBlankFCB(dstDirBlock, blank);
			if (ok < 0) return ok;

			*blank = *srcFCB;
			strcpy(blank->FileName, SrcName);
		}

		// 若移动的是目录，要更新其 ".." 指针为新父目录 dstDirBlock
		if (srcIsDir && srcFCB->Addr > 0) {
			FCB* first = (FCB*)Disk[srcFCB->Addr];
			// 按你初始化约定：第一个目录项就是 ".."
			if (strcmp(first->FileName, "..") == 0) {
				first->Addr = dstDirBlock;
			}
		}

		// 删除源目录项（标记 0xE5）
		srcFCB->FileName[0] = (char)0xe5;
	...
	// ========= B) 目标目录不存在：只能是“目录改名” =========
	// 目标“目录名”被当作新名字（同一父目录内），只允许源是目录
	if (!srcIsDir) {
		cout << "\n目标目录不存在时，move 仅支持对子目录改名。\n";
		return -5;
	}

	// 目标名必须是合法名字，且不能包含路径（你的题意：move lin chen）
	if (strchr(comd[2], '/') != NULL) {
		cout << "\n目录改名时，第二参数必须是新目录名(不能含路径)。\n";
		return -6;
	}
	if (!IsName(comd[2])) {
		cout << "\n新目录名不合法。\n";
	...
	// 同一父目录下不能与文件或目录重名
	short clash = FindFCB(comd[2], srcParent, attrib_any, dstExistFCB);
	if (clash >= 0) {
		cout << "\n当前目录中已存在同名文件或子目录，不能改名。\n";
		return -8;
	}

	// 改名
	strcpy(srcFCB->FileName, comd[2]);
	cout << "\n子目录改名成功。\n";
	return 1;
}
int BatchComd(int k)
{
	...
	ifstream fin(comd[1]);
	if (!fin) {
		cout << "\n无法打开批处理文件 " << comd[1] << endl;
		return -2;
	}

	char line[INPUT_LEN];
	while (fin.getline(line, INPUT_LEN)) {
		// 跳过空行
